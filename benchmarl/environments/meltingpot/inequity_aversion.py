...
class MeltingpotIAEnv(MeltingpotWrapper):
    """
        MeltingPot wrapper with inequity aversion.
    ...
    def _reset(
        ...
        for agent_index in range(N):
            self.temporal_smoothed_rewards[agent_index] = 0
    ...
    def _step(
        ...
        for agent_index in range(N):
            self.temporal_smoothed_rewards[agent_index] = self.ia_gamma * self.ia_lambda * self.temporal_smoothed_rewards[agent_index] + ext_rewards[agent_index]

        # apply inequity aversion
        for i in range(N):
            for j in range(N):
                if i != j:
                    rewards[i] -= (self.ia_alpha / (N - 1)) * max(self.temporal_smoothed_rewards[j] - self.temporal_smoothed_rewards[i], 0)
                    rewards[i] -= (self.ia_beta / (N - 1)) * max(self.temporal_smoothed_rewards[i] - self.temporal_smoothed_rewards[j], 0)
