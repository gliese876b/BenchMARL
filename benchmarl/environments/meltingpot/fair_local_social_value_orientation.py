...
class MeltingpotFairLocalSVOEnv(MeltingpotWrapper):
    """
        MeltingPot wrapper with Fair & Local Social Value Orientation.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.target_svos = target_svos if len(target_svos) > 0 else [0] * len(self.agent_names)
        self.svo_lambda = svo_lambda
        self.svo_gamma = svo_gamma
    ...
    def _step(
        ...
        for i in range(N):
            self.temporal_smoothed_rewards[i] = self.svo_gamma * self.svo_lambda * self.temporal_smoothed_rewards[i] + ext_rewards[i]
        ...
        # apply fair&local svo
        for i in range(N):
            total_age = sum(self.normalized_temporal_smoothed_rewards_pov_age[i].values())

            r_minus_i = (1.0 / (N - 1)) * (sum(self.normalized_temporal_smoothed_rewards_pov[i].values()) - self.normalized_temporal_smoothed_rewards_pov[i][i])
            reward_angle_i = np.rad2deg(np.arctan( (r_minus_i / self.normalized_temporal_smoothed_rewards_pov[i][i]) if self.normalized_temporal_smoothed_rewards_pov[i][i] != 0 else 0 ))
            rewards[i] -= self.svo_weight * self.social_drive_modifiers[i] * abs(self.target_svos[i] - reward_angle_i)
