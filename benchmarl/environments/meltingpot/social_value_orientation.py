...
class MeltingpotSVOEnv(MeltingpotWrapper):
    """
        MeltingPot wrapper with Social Value Orientation.
    ...
    def _reset(
        ...
        for agent_index in range(N):
            self.temporal_smoothed_rewards[agent_index] = 0.0000001 # to avoid division by zero errors
    ...
    def _step(
        ...
        for i in range(N):
            self.temporal_smoothed_rewards[i] = max(self.svo_gamma * self.svo_lambda * self.temporal_smoothed_rewards[i] + ext_rewards[i], 0.0000001) # to avoid division by zero errors

        # apply svo
        for i in range(N):
            r_minus_i = (1.0 / (N - 1)) * (sum(self.temporal_smoothed_rewards.values()) - self.temporal_smoothed_rewards[i])
            reward_angle_i = np.rad2deg(np.arctan(r_minus_i / (self.temporal_smoothed_rewards[i])))
            rewards[i] -= self.svo_weight * abs(self.target_svos[i] - reward_angle_i)
